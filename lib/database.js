  ...
  try {
    const [rows] = await connection.execute(`
      SELECT 
        ...
        COALESCE(SUM(cc.sum_amount), 0) as total_cached_amount
      FROM offers o
      LEFT JOIN offer_verticals ov ON o.offer_id = ov.offer_id
      LEFT JOIN verticals v ON ov.vertical_id = v.id
      LEFT JOIN cached_conversions cc ON o.offer_id = cc.offer_id
      GROUP BY o.offer_id, o.offer_name, o.mode, o.trigger_amount, o.low_event_type, o.high_event_type, v.name, v.id, v.payout_threshold
      ORDER BY o.offer_id
    `);
